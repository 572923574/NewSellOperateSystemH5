  ...
  <div>
    ...
    <div class="formRow">
      <div class="formRowItem">
        ...
        <el-select
          class="nameInput rowInput"
          v-model="propsData.inOutDepotTypeId"
          :placeholder="placeholderObj.inOutDepotTypeId"
        >
          <el-option
            v-for="item in inOutDepotTypeList"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          ></el-option>
    ...
    <Dialog :dialogData="dialogData" ref="selectGoodsDialog" @emitSaveFn="getTableDataFn">
      <InOutDetailTable
        ref="InOutDetailTable"
        :slot="dialogData.dialogContent"
        :goodsList="goodsList"
        :detailArray="InoutDetailList"
      ></InOutDetailTable>
    </Dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    inOutDepotTypeList: function() {
      ...
      // 出入库类型名称集合
      let inOutDepotTypeList = inOutDepotTypeData.filter(function(
        inOutDepotType
      ) {
  ...
  methods: {
    selectInOutTypeFn: function(value) {
      ...
      // 重置选择的出入库类型
      this.propsData.inOutDepotTypeId =
        this.inOutDepotTypeList && this.inOutDepotTypeList.length
          ? this.inOutDepotTypeList[0].id
          : "";
    },
      ...
      Api.goodsList(
        ...
        function(data) {
          ...
          if (!that.propsData.inOutDepotTypeId) {
            //新增单据时默认选择一个出入库类型
            that.propsData.inOutDepotTypeId =
              that.inOutDepotTypeList && that.inOutDepotTypeList.length
                ? this.inOutDepotTypeList[0].id
                : "";
          } else {
